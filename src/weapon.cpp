 ...
 * Gun configurations
 */
Gun::Gun(const WeaponProperties &properties) : Weapon(properties) {}
  ...
  if (counter >= (*this)._wProperties._fire_rate) {
	...
	case ProjectileType::NORMAL:
	  cartridge.push_back(make_shared<NormalProj>(
		  ...
		  player.get_sprite().getPosition().y * 0.5, player.get_world(),
		  new Circle(3, 10000), "", Color::Red, (*this)._wProperties._damage));
	...
	double rotation =
		player.get_sprite().getRotation() * static_cast<float>(M_PI) / 180.f -
